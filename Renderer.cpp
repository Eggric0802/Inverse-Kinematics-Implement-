...
//------------------------------------------------------------------------------------------------
nanogui::detail::FormWidget<float>* BX = nullptr;
nanogui::detail::FormWidget<float>* BY = nullptr;
nanogui::detail::FormWidget<float>* BZ = nullptr;

nanogui::detail::FormWidget<float>* PX = nullptr;
nanogui::detail::FormWidget<float>* PY = nullptr;
nanogui::detail::FormWidget<float>* PZ = nullptr;

nanogui::detail::FormWidget<float>* YX = nullptr;
nanogui::detail::FormWidget<float>* YY = nullptr;
nanogui::detail::FormWidget<float>* YZ = nullptr;

nanogui::detail::FormWidget<float>* End_x = nullptr;
nanogui::detail::FormWidget<float>* End_y = nullptr;
nanogui::detail::FormWidget<float>* End_z = nullptr;
//------------------------------------------------------------------------------------------------
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	//------------------------------------------------------------------------------------------------
		gui_1->addGroup("Bones Rotation");
	BX = gui_1->addVariable("Blue X", m_bone_animation->rotation_degree_vector[3][2]);
	BY = gui_1->addVariable("Blue Y", m_bone_animation->rotation_degree_vector[3][0]);
	BZ = gui_1->addVariable("Blue Z", m_bone_animation->rotation_degree_vector[3][1]);

	PX = gui_1->addVariable("Purple X", m_bone_animation->rotation_degree_vector[2][2]);
	PY = gui_1->addVariable("Purple Y", m_bone_animation->rotation_degree_vector[2][0]);
	PZ = gui_1->addVariable("Purple Z", m_bone_animation->rotation_degree_vector[2][1]);

	YX = gui_1->addVariable("Yellow X", m_bone_animation->rotation_degree_vector[1][2]);
	YY = gui_1->addVariable("Yellow Y", m_bone_animation->rotation_degree_vector[1][0]);
	YZ = gui_1->addVariable("Yellow Z", m_bone_animation->rotation_degree_vector[1][1]);


	gui_1->addButton("Reset Bones", []() {
		m_bone_animation->reset();
		YX->setValue(0); YY->setValue(0); YZ->setValue(30);
		PX->setValue(0); PY->setValue(0); PZ->setValue(30);
		BX->setValue(0); BY->setValue(0); BZ->setValue(30);
	});

	gui_1->addGroup("Target");
	static auto Run = gui_1->addVariable("Start Running", m_bone_animation->Run);

	static auto goal_X = gui_1->addVariable("Goal x", m_bone_animation->goal[0]);
	static auto goal_Y = gui_1->addVariable("Goal y", m_bone_animation->goal[1]);
	static auto goal_Z = gui_1->addVariable("Goal z", m_bone_animation->goal[2]);


	End_x = gui_1->addVariable("Current End X", m_bone_animation->efr.x);
	End_y = gui_1->addVariable("Current End Y", m_bone_animation->efr.y);
	End_z = gui_1->addVariable("Current End Z", m_bone_animation->efr.z);
	//------------------------------------------------------------------------------------------------
...
void Renderer::display(GLFWwindow* window)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		YX->setValue(m_bone_animation->rotation_degree_vector[1][2]); YY->setValue(m_bone_animation->rotation_degree_vector[1][0]); YZ->setValue(m_bone_animation->rotation_degree_vector[1][1]);
		PX->setValue(m_bone_animation->rotation_degree_vector[2][2]); PY->setValue(m_bone_animation->rotation_degree_vector[2][0]); PZ->setValue(m_bone_animation->rotation_degree_vector[2][1]);
		BX->setValue(m_bone_animation->rotation_degree_vector[3][2]); BY->setValue(m_bone_animation->rotation_degree_vector[3][0]); BZ->setValue(m_bone_animation->rotation_degree_vector[3][1]);



		End_x->setValue(m_bone_animation->efr.x);
		End_y->setValue(m_bone_animation->efr.y);
		End_z->setValue(m_bone_animation->efr.z);
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	glm::mat4 goal_obj_mat = glm::mat4(1.0f);
	goal_obj_mat = glm::translate(goal_obj_mat, m_bone_animation->goal);
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(goal_obj_mat));
	bone_obj->obj_color = glm::vec4(0.0, 0.7, 0.0, 1);
	draw_object(shader, *bone_obj);


	int i = 0;
	for (vector<mat4>::iterator it = m_bone_animation->boneMat.begin();
		it != m_bone_animation->boneMat.end(); it++, i++) {
		glm::mat4 obj_mat = *it;
	...
	//YX->setValue(m_bone_animation->rotation_degree_vector[1][2]); YY->setValue(m_bone_animation->rotation_degree_vector[1][0]); YZ->setValue(m_bone_animation->rotation_degree_vector[1][1]);
	//PX->setValue(m_bone_animation->rotation_degree_vector[2][2]); PY->setValue(m_bone_animation->rotation_degree_vector[2][0]); PZ->setValue(m_bone_animation->rotation_degree_vector[2][1]);
	//PX->setValue(m_bone_animation->rotation_degree_vector[3][2]); PY->setValue(m_bone_animation->rotation_degree_vector[3][0]); PZ->setValue(m_bone_animation->rotation_degree_vector[3][1]);
